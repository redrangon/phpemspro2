		...
		<div class="pep panels">
			<div class="col-xs-12 nopadding">
					...
					<div class="panel-body">
						<ul class="list-group">
							<li class="list-group-item">
								...
								<h4>
									{x2;$examsession['esdata']['name']}
								</h4>
								<span class="intros">总分：{x2;$examsession['esdata']['setting']['papersetting']['score']}分</span>
								<span class="intros">时间：{x2;$examsession['esdata']['setting']['papersetting']['papertime']}分钟</span>
								<span class="intros">及格分：{x2;$examsession['esdata']['setting']['papersetting']['passscore']}分</span>
								<span class="intros">开始时间：{x2;date:$examsession['esdata']['starttime'],'Y-m-d H:i:s'}</span>
							</li>
						</ul>
					</div>
				</div>
				...
				<p class="text-center pagebox">
					<a class="btn btn-primary openvideo" onclick="javascript:clearStorage();" data-paperid="">开始考试</a>
				</p>
				{x2;else}
				<div class="panel panel-default pagebox border">
					<div class="panel-heading bold">
					...
					<div class="panel-body">
						<ul class="list-group">
							...
							<li class="list-group-item">
								<span class="pull-right">
									<a style="margin-top: 25px;" class="btn btn-primary openvideo pull-right" onclick="javascript:clearStorage();" data-paperid="{x2;v:paper['paperid']}">开始考试</a>
								</span>
								<a class="openvideo" data-paperid="{x2;v:paper['paperid']}" onclick="javascript:clearStorage();" data-paperid="{x2;v:paper['paperid']}">
		...
		<div class="modal-content">
			...
			<div class="modal-body text-center" id="modal-body">
				<form action="index.php?exam-app-exam-selectquestions" method="post" id="verifyform">
					...
					<input name="paperid" type="hidden" value="" id="videoexamid"/>
				</form>
			</div>
...
<script>
	$(function(){
		...
		$('.openvideo').on('click',function(){
			var _this = $(this);
			$('#modal').modal();
			$('#videoexamid').val(_this.data('paperid'));
			video.play();
		});
