...
class ajax extends master
{
    ...
    public function selected()
    {
        ...
        $eles = points::getPointsBySubjectid($subjectid);
        $points = array();
        foreach($eles['points'] as $ele)
        {
            foreach($ele as $p)
            {
                $points[$p['pointid']] = $p;
    ...
    public function selectquestions()
    {
        ...
        if(!$search['questionisrows'])
        {
			...
			$args[] = array("AND","questionsubject = :questionsubject",'questionsubject',$subjectid);
            if($search['keyword'])
            {
        ...
        else
        {
			...
			$args[] = array("AND","qrsubject = :qrsubject",'qrsubject',$subjectid);
            if($search['keyword'])
            {
	...
	public function selectpointquestions()
    {
		$subjectid = $_SESSION['subjectid'];
        $subject = points::getSubjectById($subjectid);
		$pointid = \route::get('pointid');
		$questionids = question::getQuestionIdsByPoint($subject['subjectdb'],$pointid);
		$qrids = question::getQuestionRowsIdsByPoint($subject['subjectdb'],$pointid);
		$rs = array();
		foreach($questionids as $questype => $tids)
		{
			$tmp = array();
			foreach($tids as $ids)
			{
			...
			$rs[$questype]['questions'] = implode(',',$tmp);
			$rs[$questype]['number'] = count($tmp);
		}
		
		foreach($qrids as $questype => $tnids)
		{
			$tmp = array();
			foreach($tnids as $nids)
			{
				foreach($nids as $tnumber => $ids)
				{
					foreach($ids as $id)
					{
						$tmp[] = $id;
						$rs[$questype]['number'] += $tnumber;
					}
				}
			}
			$rs[$questype]['questionrows'] = implode(',',$tmp);
		}
		exit(json_encode($rs));
	}
