...
class search extends mobile
{
    ...
    public function result()
    {
        ...
        {
            $keywords = str_replace(',',' +',\PHPEMSPRO\word::getInstance()->analy($this->search['keywords'],10));
            if($this->search['questiontype'])
            {
                $args = array();
                $args[] = array("AND","qrstatus = 1");
                $args[] = array("AND","qrsubject = :subjectid","subjectid",$this->subject['subjectid']);
                //if($keywords)$args[] = array("AND","MATCH (`qrtags`) AGAINST (:qrtags IN BOOLEAN MODE)","qrtags",'+'.$keywords);
				if($keywords)$args[] = array("AND","qrquestion LIKE :qrquestion","qrquestion","%{$keywords}%");
            ...
            else
            {
                ...
                if($keywords)$args[] = array("AND","question LIKE :question","question","%{$keywords}%");
        ...
        \PHPEMSPRO\tpl::getInstance()->display('search_result');
    }
