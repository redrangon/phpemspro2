...
class favor
{
    ...
    static function getNoteList($dbid,$args,$page,$number = \PHPEMSPRO\config::webpagenumber,$orderby = 'noteid desc')
    {
        $data = array(
            'table' => 'notes',
            'query' => $args,
    ...
    {
        $r = json_decode(\PHPEMSPRO\pedis::getInstance()->getHashData('records',$username.'-'.$dbid),true);
        if(!$r)
        {
            $r = self::getRecordByUsername($dbid,$username);
            \PHPEMSPRO\pedis::getInstance()->setHashData('records',$username.'-'.$dbid,json_encode($r));
        }
        return $r;
    }
    ...
    {
        \PHPEMSPRO\pedis::getInstance()->setHashData('records',$username.'-'.$dbid,json_encode($args));
        if(rand(1,10) == 5)
        {
    ...
    static function getFavorList($dbid,$args,$page,$number = \PHPEMSPRO\config::webpagenumber,$orderby = 'favorid desc')
    {
        $data = array(
            'table' => 'favors',
            'query' => $args,
    ...
    static function getExamHistoryList($dbid,$args,$page,$number = \PHPEMSPRO\config::webpagenumber,$fields = false,$orderby = 'ehid desc')
    {
        $data = array(
            'select' => $fields,
            'table' => 'examhistory',
    ...
    static function getExamHistoryNumber($dbid,$args)
    {
        $data = array(
            'select' => "count(*) as number",
            'table' => 'examhistory',
    ...
    static function getUserExamHistoryList($dbid,$args,$page,$number = \PHPEMSPRO\config::webpagenumber,$orderby = 'ehid desc')
    {
        $data = array(
            'table' => 'examhistory',
            'query' => $args,
