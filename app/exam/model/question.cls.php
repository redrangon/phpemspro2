...
class question
{
    ...
    static function getQuestionList($db,$args,$page,$number = \PHPEMSPRO\config::webpagenumber,$orderby = 'questionid desc')
    {
        $data = array(
            'table' => 'questions',
            'query' => $args,
        ...
        return \PHPEMSPRO\pepdo::getInstance($db)->listElements($page,$number,$data);
    }

    static function getQuestionRowsList($db,$args,$page,$number = \PHPEMSPRO\config::webpagenumber,$orderby = 'qrid desc')
    {
        $data = array(
            'table' => 'questionrows',
            'query' => $args,
    ...
    {
        $result = json_decode(\PHPEMSPRO\pedis::getInstance()->getHashData('subjectquestions',$db.'-'.$pointid),true);		
        if($result)
        {
            return $result;
        }
        ...
        $r = \PHPEMSPRO\pepdo::getInstance($db)->getElements($data);
        $rs = array();
        foreach($r as $p)
        {
            $rs[$p['questiontype']][$p['questionlevel']][] = $p['questionid'];
        }
        \PHPEMSPRO\pedis::getInstance()->setHashData('subjectquestions',$db.'-'.$pointid,json_encode($rs));
        return $rs;
    }

    static function getQuestionRowsIdsByPoint($db,$pointid)
    {
        $result = json_decode(\PHPEMSPRO\pedis::getInstance()->getHashData('subjectquestionrows',$db.'-'.$pointid),true);
        if($result)
        {
            return $result;
        }
        ...
        \PHPEMSPRO\pedis::getInstance()->setHashData('subjectquestionrows',$db.'-'.$pointid,json_encode($rs));
        return $rs;
    }
    ...
    static function getQuestionById($db,$questionid)
    {
        ...
        return \PHPEMSPRO\pepdo::getInstance($db)->getElement($data);
    }

    static function getQuestionsByArgs($db,$args,$orderby = "questionparent asc,questionorder asc,questionid desc",$select = "questionid,question,questiontype,questionselect,questionselectnumber,questionanswer,questionintro,questionlevel,questionpoints,questionparent")
    {
        $data = array(
            'select' => $select,
            'table' => 'questions',
        ...
        return \PHPEMSPRO\pepdo::getInstance($db)->getElements($data);
    }

    static function formatQuestion($question,$role = false)
    {
        if($role)return $question;
        $question['questionselect'] = strings::parseSelector($question['questionselect']);
        return $question;
    }
    static function formatQuestions($questions,$role = false)
    {
        if($role)return $questions;
        foreach($questions as $key => $question)
        {
            $questions[$key]['questionselect'] = strings::parseSelector($question['questionselect']);
        }
        return $questions;
    }

    static function getQuestionRowsByArgs($db,$args,$orderby = "qrid desc",$select = "qrid,qrquestion,qrtype,qrlevel,qrpoints,qrnumber")
    {
        $data = array(
            'select' => $select,
            'table' => 'questionrows',
    ...
    static function getQuestionRowsById($db,$qrid,$withchilid = true)
    {
        ...
        $r = \PHPEMSPRO\pepdo::getInstance($db)->getElement($data);
        if($withchilid)
    ...
    {
        if($args['question'] && $args['questionselect'])
        {
            $args['questiontags'] = word::getInstance()->analy($args['question'].$args['questionselect']);
        }
        $data = array(
            'table' => 'questions',
            'value' => $args,
        ...
        \PHPEMSPRO\pepdo::getInstance($db)->updateElement($data);
    }
    ...
    static function modifyQuestionRows($db,$id,$args)
    {
        if($args['qrquestion'])
        {
            $args['qrtags'] = word::getInstance()->analy($args['qrquestion']);
        }
        $data = array(
            'table' => 'questionrows',
            'value' => $args,
    ...
    static function addQuestion($db,$args)
    {
        ...
        return \PHPEMSPRO\pepdo::getInstance($db)->insertElement($data);
    }
    ...
    static function hideQuestion($db,$id)
    {
        ...
        return \PHPEMSPRO\pepdo::getInstance($db)->updateElement($data);
    }
    ...
    static function clearRecyle($db,$subjectid)
    {
        $data = array(
            'table' => 'questions',
            'query' => array(
                array("AND","questionsubject = :questionsubject","questionsubject",$subjectid),
                array("AND","questionstatus = 0")
            )
        );
		\PHPEMSPRO\pepdo::getInstance($db)->delElement($data);
        $data = array(
            'table' => 'questionrows',
            'query' => array(
                array("AND","qrsubject = :qrsubject","qrsubject",$subjectid),
                array("AND","qrstatus = 0")
            )
        );
        \PHPEMSPRO\pepdo::getInstance($db)->delElement($data);
        return true;
    ...
    static function delQuestion($db,$id)
    {
        ...
        $r = \PHPEMSPRO\pepdo::getInstance($db)->delElement($data);
    ...
    static function delQuestionRows($db,$id)
    {
        ...
        return \PHPEMSPRO\pepdo::getInstance($db)->delElement($data);
    }
    ...
    static function importQuestions($uploadfile,$subject,$username)
    {
            ...
            if(count($question) >= 6)
            {
                ...
                if($isqr)
                {
                    ...
                    if($istitle)
                    {
                        ...
                        $args['qrtime'] = TIME;
                        $args['qrauthor'] = $username;
                        $args['qrpoints'] = explode(',',$question[6]);
                        $args['qrsubject'] = $subject['subjectid'];
                        $qrid = self::addQuestionRows($subject['subjectdb'],\PHPEMSPRO\route::addSlashes($args));
                    ...
                    else
                    {
                        ...
                        $id = self::addQuestion($subject['subjectdb'],\PHPEMSPRO\route::addSlashes($args));
    ...
    static function importCsvQuestions($file)
    {
        ...
        while ($data = fgetcsv($handle))
        {
            ...
            {
                $question[$key] = iconv("GBK","UTF-8//IGNORE",$p);
            }
            if(count($question) >= 5)
            {
                ...
                if($isqr)
                {
                    ...
                    if($istitle)
                    {
                        ...
                        $targs['qrquestion'] = trim(nl2br($question[1])," \n\t");
                    ...
                    else
                    {
                        ...
                        $targs['question'] = trim(nl2br($question[1])," \n\t");
                        $targs['questionselect'] = trim(nl2br($question[2])," \n\t");
                        if(!$targs['questionselect'] && $targs['questiontype'] == 3)
                        {
                            $targs['questionselect'] = '<p>A、对<p><p>B、错<p>';
                        }
                        $targs['questionselectnumber'] = $question[3];
                        $targs['questionanswer'] = trim($question[4]," \n\t");
                        $targs['questiondescribe'] = trim($question[5]," \n\t");
    ...
    static function exportQuestions($db,$args)
    {
        ...
        foreach($questions as $p)
        {
            $r[] = array(
                ...
                'question' => iconv("UTF-8","GBK//IGNORE",html_entity_decode($p['question'])),
                'questionselect' => iconv("UTF-8","GBK//IGNORE",html_entity_decode($p['questionselect'])),
                'questionselectnumber' => iconv("UTF-8","GBK//IGNORE",$p['questionselectnumber']),
                'questionanswer' => iconv("UTF-8","GBK//IGNORE",html_entity_decode($p['questionanswer'])),
                'questionintro' => iconv("UTF-8","GBK//IGNORE",html_entity_decode($p['questionintro'])),
    ...
    static function exportQuestionRows($db,$args)
    {
        ...
        foreach($questionrows as $p)
        {
            ...
            foreach($rs as $p)
            {
                $r[] = array(
                    'questiontype' => $p['questiontype'],
                    'question' => html_entity_decode(iconv("UTF-8","GBK//IGNORE",$p['question'])),
                    'questionselect' => html_entity_decode(iconv("UTF-8","GBK//IGNORE",$p['questionselect'])),
                    'questionselectnumber' => iconv("UTF-8","GBK",$p['questionselectnumber']),
                    'questionanswer' => html_entity_decode(iconv("UTF-8","GBK//IGNORE",$p['questionanswer'])),
                    'questionintro' => html_entity_decode(iconv("UTF-8","GBK//IGNORE",$p['questionintro'])),
                    'questionpoints' => implode(',',$p['questionpoints']),
                    'questionlevel' => $p['questionlevel'],
                    'isrows' => '1',
                    'istitle' => 0
    ...
    static function getExamQuestionNumber($db,$setting)
    {
        ...
        if($questions)
        {
            ...
            $sql = \PHPEMSPRO\pepdo::getInstance($db)->makeSelect($data);
            $r = \PHPEMSPRO\pepdo::getInstance($db)->fetch($sql);
    ...
    {
        $numbers = json_decode(\PHPEMSPRO\pedis::getInstance()->getHashData('number',$db),true);
        if($numbers[$pointid])return $numbers[$pointid];
        else
        {
            $data = array('count(*) as number','questions',array(array("AND","find_in_set(:questionpoints,questionpoints)",'questionpoints',$pointid),array("AND","questionstatus = 1"),array("AND","questionparent = 0")));
            $sql = \PHPEMSPRO\pepdo::getInstance($db)->makeSelect($data);
            $r = \PHPEMSPRO\pepdo::getInstance($db)->fetch($sql);
            $number = intval($r['number']);
            ...
            \PHPEMSPRO\pedis::getInstance()->setHashData('number',$db,json_encode($numbers));
            return $number;
        }
    }
    ...
    {
        $questionids = json_decode(\PHPEMSPRO\pedis::getInstance()->getHashData('questions',$db),true);
        $qrids = json_decode(\PHPEMSPRO\pedis::getInstance()->getHashData('questionrows',$db),true);
        if(!$questionids[$pointid])
        {
            $ids = array();
            $data = array('questionid','questions',array(array("AND","find_in_set(:questionpoints,questionpoints)",'questionpoints',$pointid),array("AND","questionstatus = 1"),array("AND","questionparent = 0")),false,false,false);
            $sql = \PHPEMSPRO\pepdo::getInstance($db)->makeSelect($data);
            $r = \PHPEMSPRO\pepdo::getInstance($db)->fetchAll($sql);
            foreach($r as $p)
            {
                $ids[] = $p['questionid'];
            }
            $questionids[$pointid] = $ids;
            \PHPEMSPRO\pedis::getInstance()->setHashData('questions',$db,json_encode($questionids));
        }
        if(!$qrids[$pointid])
        {
            $ids = array();
            ...
            $qrids[$pointid] = $ids;
            \PHPEMSPRO\pedis::getInstance()->setHashData('questionrows',$db,json_encode($qrids));
        }
    ...
    static function correctpaper($question,$paper,$username)
    {
        $scorelist = array();
                ...
                foreach($qs as $q) {
                    if ($question[$q['questionid']]) {
                        if ($questypes[$key]['questchoice'] == 3) {
                            if(is_array($question[$q['questionid']]))$answer = implode('', $question[$q['questionid']]);
                            else $answer = $question[$q['questionid']];
        ...
        return array($args,$needteacher);
    }

    static function monitorpaper($dbid,$sessionvars)
    {
        $question = $sessionvars['useranswer'];
        $paper = $sessionvars;
        $username = $sessionvars['username'];
        list($args,$needteacher) = self::correctpaper($question,$paper,$username);
    ...
    {
        if(\PHPEMSPRO\route::get('sign') != md5($paper['token'].'-'.$paper['sign'].'-'.$username.'-'.\PHPEMSPRO\config::secretkey))
        {
            $message = array(
                'statusCode' => 300,
    ...
    static function getErrorsList($args,$page,$number = \PHPEMSPRO\config::webpagenumber,$orderby = 'erstatus asc,erid desc')
    {
        $data = array(
            'table' => 'errors',
            'query' => $args,
