...
namespace PHPEMSPRO\core\model;
...
class category
{
	...
	static public function getCategoryList($args,$page,$number = \PHPEMSPRO\config::webpagenumber,$orderby = 'catorder desc')
	{
        $data = array(
            'table' => 'category',
            'query' => $args,
	...
	static public function getCategoriesByArgs($args,$orderby = 'catorder desc,catid desc')
	{
        $data = array(
            'table' => 'category',
            'query' => $args,
    ...
    static public function getAllCategory($appcode)
	{
		if(self::$categories[$appcode] === null)
		{
            ...
            self::$categories[$appcode] = \PHPEMSPRO\pepdo::getInstance()->getElements($data);
    ...
    static public function levelCategory(&$t,$index,$allcats,$selected,$hrefpre)
	{
		if(is_array($allcats[$index]))
		{
			foreach($allcats[$index] as $p)
			{
                ...
                $t[$p['catid']] = array('text' => $p['catname'],'href' => $hrefpre.$p['catid'],'color' => '#FFFFFF',"backColor" => '#374850');
                else
