...
class model
{
    static function getModels($page,$number = \PHPEMSPRO\config::webpagenumber)
    {
        $args = array(
            'select' => false,
            'table' => 'models',
        ...
        return \PHPEMSPRO\pepdo::getInstance()->listElements($page,$number,$args);
    }
    ...
    static function getPropertyiesByModelcode($modelcode)
    {
        ...
        return \PHPEMSPRO\pepdo::getInstance()->getElements($args);
    }

    static function getAssignPropertiesByModelcode($modelcode,$fields = false)
    {
        if(!$fields)return false;
        $properties = self::getPropertyiesByModelcode($modelcode);
        foreach($properties as $key => $p)
        {
            if(!in_array($p['ppyfield'],$fields))
            {
