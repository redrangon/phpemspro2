...
{public function logout()
    ...
    {
        if(\PHPEMSPRO\route::get('userfindpassword'))
        {
        ...
        {
            \PHPEMSPRO\tpl::getInstance()->display('login_findpassword');
        }
    }
    ...
    public function register()
    {
        if(\PHPEMSPRO\route::get('userregister'))
        {
            ...
            \PHPEMSPRO\session::getInstance()->setSessionUser(array('sessionuserid'=>$id,'sessionpassword'=>md5($args['userpassword']),'sessionip'=>\PHPEMSPRO\route::getClientIp(),'sessiongroupcode' => $user['usergroupcode'],'sessionlogintime'=>TIME,'sessionusername'=>$username));
            $message = array(
                'statusCode' => 200,
                "message" => "操作成功",
                "callbackType" => 'forward',
        ...
        if(\PHPEMSPRO\route::get('userlogin'))
        {
            ...
            if($user)
            {
                if($user['userpassword'] == md5($args['userpassword']))
                {
					...
					$referurl = $_COOKIE['referurl'];
                    $message = array(
                        'statusCode' => 200,
                        "message" => "登陆成功",
                        "callbackType" => "forward",
                        "forwardUrl" => $referurl?"index.php".$referurl:"index.php",
                        'callback' => 'cleardata'
                    );
                    \PHPEMSPRO\route::urlJump($message);
                }
