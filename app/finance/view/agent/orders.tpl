			...
			<div id="datacontent">
				...
				<div class="panel panel-default" id="datacontent-container">
					...
					<div class="panel-body">
						<form action="index.php?finance-agent-orders" method="post" class="form-inline">
							<table class="table">
								<thead>
								<tr>
									<td>
									...
									<td>
										<input class="form-control" name="search[useremail]" size="10" type="text" value="{x2;$search['useremail']}"/>
									</td>
									<td>
										手机号码：
									</td>
									<td>
										<input class="form-control" name="search[userphone]" size="10" type="text" value="{x2;$search['userphone']}"/>
									</td>
									<td>
										订单状态：
									</td>
						...
						<table class="table table-hover table-bordered">
							...
							<tr>
								...
								<td>{x2;v:order['orderrealname']}</td>
								<td>{x2;v:order['orderemail']}</td>
								<td>{x2;v:order['orderprice']}</td>
								<td>{x2;v:order['ordertime']}</td>
								<td>{x2;$orderstatus[v:order['orderstatus']]}</td>
								<td>
									{x2;if:(TIME - strtotime(v:order['ordertime'])) < 432000 && v:order['orderstatus'] == 2}
									<a href="index.php?finance-agent-basics-cancel&ordersn={x2;v:order['ordersn']}" class="confirm" msg="确定要取消吗？">取消</a>
									{x2;else}
									-
									{x2;endif}
								</td>
