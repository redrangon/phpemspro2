...
class basics extends mobile
{
    ...
    public function openpackage()
    {
        ...
        $useremail = \PHPEMSPRO\route::get('email');
        if(!$useremail)$useremail = $userphone.'@139.com';
        ...
        $title = null;
        $number = 0;
        $packages = \PHPEMSPRO\route::get('packages');
        foreach($packages as $trid => $ids)
        {
        ...
        $args['ordersurplus'] = $coin;
        \PHPEMSPRO\finance\model\orders::addOrder($args);
        \PHPEMSPRO\finance\model\orders::payforOrder($args['ordersn']);
        users::modifyUser($this->user['userid'],array('usercoin' => $coin));
        $message = array(
            'statusCode' => 200,
            "message" => '购买成功',
            "callbackType" => "forward",
            "forwardUrl" => "index.php?finance-mobile-orders"
        );
        \PHPEMSPRO\route::urlJump($message);
    }
    ...
    public function openbasic()
    {
        ...
        $title = null;
        $number = 0;
        $basics = \PHPEMSPRO\route::get('basics');

        foreach($basics as $db => $ids)
        {
            foreach($ids as $basicid => $id)
            {
                foreach($id as $type => $p)
                {
                    $basic = exams::getBasicById($db,$basicid);
                    if(!$title)$title = $basic['basic'];
                    $subject = points::getSubjectById($basic['basicsubject']);
        ...
        $args = array();
        $args['ordersaleprice'] = floatval($orderprice);
        $args['orderprice'] = round(floatval($args['ordersaleprice'] * $this->rate),2);
        if(floatval($this->user['usercoin']) < $args['orderprice'])
        {
            $message = array(
        ...
        $args['ordername'] = "{$title}等{$number}个考场";
        $args['orderitems'] = $items;
        $args['ordersn'] = date('YmdHi').rand(100,999);
        $args['orderusername'] = $u['username']?$u['username']:$userphone;
        $args['orderrealname'] = $userrealname;
    ...
    public function package()
    {
        ...
        if($this->search['trid'])
        {
            $args[] = array("AND","trid = :trid","trid",$this->search['trid']);
        }
    ...
    public function basics()
    {
        ...
        \PHPEMSPRO\tpl::getInstance()->display('basics_basics');
    }
    ...
    public function training()
    {
        ...
        $trainings = training::getTrainingsByArgs($args);
        \PHPEMSPRO\tpl::getInstance()->assign('trainings',$trainings);
        \PHPEMSPRO\tpl::getInstance()->display('basics_training');
    }
