...
'zoombox':(function(){
	...
	return {'show':function(type,obj,url){
			switch(type)
			{
				...
				if(nimg.width < 600)return false;
...
function submitAjax(parms){
	...
	$.currentAjax = $.ajax({"url":parms.url,
		"type":parms.method?parms.method:'post',
		"data":parms.query,
...
function formsubmit(){
	...
	if($(_this).attr('method') && $(_this).attr('method').toUpperCase() == 'GET')
	{
		...
		url += '&'+query;
		if($(_this).attr('action-before'))eval($(_this).attr('action-before'))();
		window.location = url;
	}
	else
	{
		submitAjax({"url":$(_this).attr('action'),"async":false,"query":query+'&userhash='+Math.random(),"target":target,'action-before':$(_this).attr('action-before')});	
	}
	return false;
}
	...
	if($(_this).attr('etype') == 'simple')
	{
		var config = {
			...
			[
				...
				['Table','HorizontalRule','SpecialChar','Mathjax','Image']
			]
		};
...
$(function(){
	...
	(function(){
		...
		if(search.indexOf('user-app-login') < 0 && search.indexOf('wxlogin-app') < 0)
		{
			$.cookie('referurl',search);
		}
	})();
});