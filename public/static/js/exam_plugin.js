...
function storage(token)
{
	...
	this.initData = function(){
		var data = $.parseJSON(this.storage.getItem('questions'));
		var tmp = {};
		for(x in data)
		{
...
function saveanswer(savesession){
	var url = $('#exampaper').attr('action')+'answer';
	if(savesession)url = $('#exampaper').attr('action')+'session';
	var params = {
		url:url,
		query:$('#exampaper :input').serialize()
	};
	submitAjax(params);
}
...
function markQuestion(rel,isTextArea)
{
	...
	try
	{
		f = $('#exampaper .panel[data-questionid='+rel+'] :input');
	}catch(e)
	{
		f = false;
	}
	...
	if(isTextArea)
	{
		if($('#exampaper .panel[data-questionid='+rel+'] :input').val() && $('#exampaper .panel[data-questionid='+rel+'] :input').val() != '' && $('#exampaper .panel[data-questionid='+rel+'] :input').val() != '<p></p>')t++;
	}
	else
	$('#exampaper .panel[data-questionid='+rel+'] :input').each(function(){if($(this).is(':checked') && $(this).val() && $(this).val() != '' && $(this).val() != '<p></p>')t++;});
	if(t > 0)
	{
