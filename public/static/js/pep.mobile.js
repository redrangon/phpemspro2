...
var pep = {
    ...
    'ajaxSending':false,
    'events':[],
    'injectEvent':function(func){
        pep.events.push(func);
    },
	'clearEvents':function(){
		pep.events.map(function(func,index){
            func();
        })
	},
    'initpage':function(page,tabindex,initscroll){
        ...
        if(tabindex >= 0)
        {
            var cp = page.find('.pages-tabs').hide().eq(tabindex).css('display','block');
        ...
        if(cnt.attr('data-scroll') == 'yes')
        {
            ...
            cnt.scroll(function(){
                ...
                if(pep.isTrueVar(_this.data('pageurl')))
                {
	...
	'setPageContent':function(dom){
        var pageContents = $.parseJSON(localStorage.getItem('pageContents'));
		if(!pageContents)pageContents = {};
		var url = window.location.pathname + window.location.search;
		var id = url.hash();
		if(dom.find('.pages-content:first').data('nocache') == 'yes')
		{
			return ;
		}
		if(dom.find('.pages-content:first').data('savehtml') == 'yes')
		{
			pageContents[id] = {content:dom.html(),'url':url,'tabindex':dom.attr('data-index'),savehtml:true};			
		}
		else
		{
			pageContents[id] = {content:null,'url':url,'tabindex':dom.attr('data-index')};
			$('body').data(id,dom);
		}
	...
	'getPageContent':function(id){
        ...
        if(!pageContents)pageContents = {};
		var id = (window.location.pathname + window.location.search).hash();
        var p = pageContents[id];
        if(!p || (new Date().getTime() - p.time > 900000))p = false;
        if(!p.savehtml && !$("body").data(id))return false;
        return p;
	},
    ...
    'goPrePage':function(){
        ...
        {
			var par = {'url':url,'tabindex':p.tabindex};
    ...
    'newPage':function(title,ispre,direction){
        ...
        var page = $('<div class="pages">'+
            '<div class="pages-tabs">'+
                '<div class="pages-header">'+
                    ...
                    '</a>'+
                '</div>'+
                '<div class="pages-content text-center padding pd15 nopdLR"></div>'+
            '</div>'+
        '</div>');
        ...
        circle.play();
        page.find(".pages-content:first").html(circle.canvas);
        page.appendTo($('body'));
        if(direction == 'left')
        {
                ...
                switch(type) {
                    ...
                    case 'randcode':
                        var cnt = $('<div class="maskbox"></div>'+
                            ...
                            '<div class="tipbox" style="clear:both;overflow:hidden;">' +
                                '<img src="index.php?core-api-index-randcode" class="randCode col-xs-5 padding pd1"/>' +
                                '<div class="col-xs-7 padding pd1">' +
                                    '<input type="text" class="form-control" style="line-height:0.8rem;height:0.8rem" id="randformcode" needle="needle" msg="您必须填写验证码" placeholder="请填写验证码">' +
                                '</div>' +
                            '</div>'+
                            '<div class="tiptitle">'+
                            '<div class="col-xs-6 confirm"> 确定 </div>'+
                    ...
                    case 'loading':
                        ...
                        var cnt = $('<div class="maskbox"></div>'+
                            ...
                            '</div>');
                        cnt.find('.loader:first').html(circle.canvas);
                        cnt.appendTo(m);
                        m.appendTo($('body'));
                        break;
...
function formsubmit(){
    ...
    if($(_this).attr('method') && $(_this).attr('method').toUpperCase() == 'GET')
    {
...
function storage(token)
{
    ...
    this.addItem = function(name,value){
        ...
        tmp[token] = this.question;
        this.storage.setItem('questions',JSON.stringify(tmp))
    }
    this.getItem = function(){
        return this.question;
    }
    this.synchData = function(data){
        var tmp = {};
        tmp[token] = data;
        this.storage.setItem('questions',JSON.stringify(tmp))
    }
    this.initData = function(){
        var data = JSON.parse(this.storage.getItem('questions'));
        var tmp = {};
        for(x in data)
        {
            if(x == token)this.question = data[x];
            else data[x] = null;
        }
        tmp[token] = this.question;
        this.storage.setItem('questions',JSON.stringify(tmp))
...
$(function(){
    ...
    $('body').delegate('.pages-footer .menu','click',function(){
        var _this = $(this);
