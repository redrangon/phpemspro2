...
class html
{
    ...
    static public function buildForm($properties,$value = null)
    {
        ...
        foreach($properties as $property)
        {
            ...
            if($property['ppyhtmltype'] != 'hidden')
            {
                ...
                $property['ppysource'] = route::stripSlashes($property['ppysource']);
    ...
    static public function buildHtml($properties,$value = null)
    {
        ...
        foreach($properties as $property)
        {
            ...
            if($property['ppyhtmltype'] != 'hidden')
            {
                ...
                if(!$property['ppyproperty'])$property['ppyproperty'] = array();
                $property['ppyproperty'][] = array('key' => 'id','value' => $property['ppyfield']);
                switch ($property['ppyhtmltype'])
                {
                    case 'radio':
                    case 'select':
                        foreach($property['ppyvalue'] as $p)
                        {
                            if($p['value'] == $value[$property['ppyfield']])
                            {
                                $tmp['html'] = $p['key'];
                                break;
                            }
                    ...
                    case 'checkbox':
                        foreach($property['ppyvalue'] as $p)
                        {
                            if(in_array($p['value'],$value[$property['ppyfield']]))
                            {
                                $tmp['html'][] = $p['key'];
                            }
                        }
                        $tmp['html'] = implode(",",$tmp['html']);
                        break;

                    case 'thumb':
                        $tmp['html'] = '<img class="img-thumbnail" style="max-width:400px;" src="'.$value[$property['ppyfield']].'"/>';
                        break;

                    case 'picture':
                        foreach($value[$property['ppyfield']] as $p)
                            $tmp['html'] .= '<img class="col-xs-4" src="'.$p.'"/>';
                        break;

                    case 'videotext':
                        $tmp['html'] = '<a target="_blank" href="'.$value[$property['ppyfield']].'">查看</a>';
                        break;

                    default:
                        $tmp['html'] = $value[$property['ppyfield']];
                        break;
                }
            }
            $htmls[] = $tmp;
    ...
    static function thumb($args,$default,$value)
    {
        if(is_array($args['properties']))
        {
            $property = array();
    	...
    	<script type="text/template" id="pe-template-{$id}">
    		<div {$property}>
            	<div class="qq-upload-button-selector" style="clear:both;max-width: 360px;">
...
EOF;
    ...
    static public function editor($args,$default,$value)
    {
        ...
        $str .= " {$property}";
        $str .= ">".htmlspecialchars($value)."</textarea>";
        return $str;
    }
    ...
    static function radio($args,$default,$value)
    {
        if(!$value)
        {
            $value = self::_autoDefault($default);
    ...
    static function checkbox($args,$default,$value)
    {
        if(!$value)
        {
