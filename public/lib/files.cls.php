...
class files
{
    ...
    static function createRandImage($randCode = NULL,$width = 60, $height = 24,$mix = 50)
    {
    	...
    	for($i = 0;$i<4;$i++)
    	{
			...
			imagettftext($image,14,intval(rand(0,60)),10+$i*$par,23+rand(3,8), $text_color,PEP_PATH.'/public/static/fonts/mistral.ttf',$randCode[$i]);
    	}
    	for($i=0;$i<250;$i++)
		{
    ...
    static function base64Img($imgurl,$fullpath = true)
    {
        if(!file_exists($imgurl))return false;
        $ext = strtolower(self::getFileExtName($imgurl));
    ...
    static function saveBase64Img($data,$ext = 'png')
    {
        $path = 'files/attach/files/face/'.date('Ymd').'/';
        self::mdir($path);
        $path .= uniqid().rand(1000,9999).'.'.$ext;
        self::writeFile($path,base64_decode($data));
        return $path;
    }
}
?>